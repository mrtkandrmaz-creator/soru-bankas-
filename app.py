...
# STREAMINGS_CHUNK: Defining comprehensive MEB Curriculum and mega metadata pools...
MEB_MUFREDAT = {
    "Matematik": [
...
# STREAMINGS_CHUNK: Initializing infinite combinatoric pools for mega quiz data sets...
MEGA_Dunya_Cografyasi = [
    ...
    ("Avrupa'nın ortasından doğup Karadeniz'e dökülen, kıtanın en uzun ikinci nehri hangisidir?", "Tunus", ["Ron Nehri", "Tuna Nehri", "Ren Nehri"]),
...
def dinamik_yapay_zekali_soru_motoru(ders, unite):
    """Gelişmiş %100 Dinamik ve Sınırsız Kombinasyonlu Soru Motoru (Kısır Döngü Önleyici)"""
    ...
    # -----------------------------------------------------
    # GENEL KÜLTÜR & MEGA BİLGİ YARIŞMASI KATEGORİLERİ
    # -----------------------------------------------------
...
# -----------------------------------------------------
# STREAMLIT ARAYÜZ (Gelişmiş Filtreleme ve Kontroller)
# -----------------------------------------------------
...
if not st.session_state["sorular_hazir"] and not st.session_state["test_aktif"] and not st.session_state["test_bitti"]:
    if secilen_uniteler:
        if st.sidebar.button("✨ Karma Test Üret", type="primary", use_container_width=True):
            with st.spinner("Milyonlarca kombinasyonlu yapay zeka motoru soruları harmanlıyor..."):
...
if not st.session_state["sorular_hazir"] and not st.session_state["test_aktif"] and not st.session_state["test_bitti"]:
    ...
    if secilen_uniteler:
        st.info(f"Seçilen Kategori/Ünite Sayısı: **{len(secilen_uniteler)}**. Sistem kısır döngüyü önleyen **sınırsız havuz ve dinamik parametrik şablon motoru** ile benzersiz sorular hazırlayacaktır.")

elif st.session_state["sorular_hazir"] and not st.session_state["test_aktif"] and not st.session_state["test_bitti"]:
    st.success("✅ Sorular zenginleştirilmiş bilgi havuzlarından ve yapay zeka şablonlarından başarıyla üretildi!")
